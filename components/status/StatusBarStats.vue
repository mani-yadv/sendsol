<template>
    <div class="flex w-full border-b border-neutral/75 py-0.5">
        <div
            class="cursor-pointer border-r border-neutral/75 px-2 text-2xs text-base-content/50 hover:text-base-content/75"
        ...
        <BottomDrawer v-if="showRoadmap" @close="showRoadmap = false">
            <template #header>
                <h3 class="text-lg font-bold">SendSol Roadmap 2025</h3>
            </template>

            <template #content>
                <div class="-ml-20 h-[70vh] overflow-y-auto">
                        ...
                        <li v-for="(item, index) in roadmapItems" :key="index">
                            ...
                            <div class="timeline-middle">
                                <svg
                                    ...
                                    fill="currentColor"
                                    class="size-5"
                                    :class="item.completed ? 'text-primary' : 'text-base-content/50'">
                                    <path
                                        fill-rule="evenodd"
                                        d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.236 4.53L8.29 10.722a.75.75 0 00-1.08 1.04l1.75 1.815a.75.75 0 001.146-.102l3.75-5.25z"
                                        clip-rule="evenodd" />
                                </svg>
                            </div>
                            <div class="timeline-end timeline-box" :class="item.completed ? 'bg-primary/10' : ''">
                                <div class="text-sm font-bold">{{ item.title }}</div>
                                <div class="mt-1 text-xs opacity-70">{{ item.description }}</div>
                            </div>
